import streamlit as st
import plotly.express as px
import pandas as pd
from sklearn.cluster import KMeans
from sklearn.manifold import TSNE
from sklearn.preprocessing import StandardScaler

def show(df):
    st.markdown("## :material/add_shopping_cart: AI MARKET SEGMENTATION")
    st.markdown("### HARDWARE CLUSTERS VS. PRICING STRATEGY")

    # Data preparation (adding processing for prices)
    ml_features = ['Process_Size_nm', 'Memory_MB', 'Bandwidth_GBs', 'TDP_Watts']
    
    # Filtering data: Both hardware parameters and price must be present to allow comparison.
    # Only look at modern graphics cards from 2016 onwards, because the price system for too old cards is different.
    df_ml = df[(df['Release_Year'] >= 2016) & (df['Brand'].isin(['NVIDIA', 'AMD', 'Intel']))].copy()
    df_ml = df_ml.dropna(subset=ml_features + ['Launch_Price'])
    
    if len(df_ml) > 30:
        
        # Control Panel (Top)
        col_ctrl1, col_ctrl2 = st.columns([1, 2])
        
        with col_ctrl1:
            # View Switcher: Core Functionality
            view_mode = st.radio(
                "COLOR SCHEME (VIEW MODE)", 
                ["AI Hardware Clusters (Specs)", "Market Price Tiers ($$$)"],
                horizontal=True
            )
            
        with col_ctrl2:
            # If it is AI mode, display the clustering slider.
            if view_mode == "AI Hardware Clusters (Specs)":
                k = st.slider("AI CLUSTERS (K)", 2, 6, 4, help="How many groups should the AI divide the GPUs into?")
            else:
                st.info("Price Tiers: Entry (<$300), Mid ($300-600), High ($600-1000), Enthusiast (>$1000).")

        # Calculation Logic
        
        # A. t-SNE projection (always based on hardware parameters)
        # No matter how it is colored, the position of a point always represents "performance/specifications". Unlike an xy coordinate graph, the vertical position does not represent any numerical magnitude.
        # In this way, if the distribution of price colors is consistent with the distribution of positions, it proves that "the pricing strategy conforms to the performance grading".
        X = df_ml[ml_features]
        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(X)
        
        # Dimensionality reduction to 2D
        tsne = TSNE(n_components=2, random_state=42, perplexity=30)
        X_embedded = tsne.fit_transform(X_scaled)
        df_ml['tsne_x'] = X_embedded[:, 0]
        df_ml['tsne_y'] = X_embedded[:, 1]
        
        # B. Two Classification Logics
        
        # Logic 1: AI K-Means (based on specifications)
        kmeans = KMeans(n_clusters=k if view_mode == "AI Hardware Clusters (Specs)" else 4, random_state=42, n_init=10)
        df_ml['AI_Cluster'] = kmeans.fit_predict(X_scaled).astype(str)
        
        # Logic 2: Price Segmentation (Based on Market)
        # Define price range
        price_bins = [0, 300, 600, 1000, 100000]
        price_labels = ['1. Entry Level (<$300)', '2. Mid-Range ($300-$600)', '3. High-End ($600-$1000)', '4. Enthusiast (>$1000)']
        df_ml['Price_Tier'] = pd.cut(df_ml['Launch_Price'], bins=price_bins, labels=price_labels)

        # Drawing
        
        # Determine the color column based on the selection
        color_col = 'AI_Cluster' if view_mode == "AI Hardware Clusters (Specs)" else 'Price_Tier'
        title_text = f"t-SNE Projection colored by {view_mode}"
        
        # Define a color mapping to make prices look more intuitive (cool colors represent cheap -> warm colors represent expensive)
        price_color_map = {
            '1. Entry Level (<$300)': '#a1c9f4',  
            '2. Mid-Range ($300-$600)': '#8de5a1', 
            '3. High-End ($600-$1000)': '#ff9f9b', 
            '4. Enthusiast (>$1000)': '#de5253'    
        }
        
        fig = px.scatter(
            df_ml, 
            x='tsne_x', 
            y='tsne_y', 
            color=color_col,
            color_discrete_map=price_color_map if view_mode == "Market Price Tiers ($$$)" else None,
            hover_data=['Name', 'Launch_Price', 'TDP_Watts', 'Release_Year'],
            title=None,
            size='TDP_Watts', # The size of the bubble represents power.
            size_max=15,
            height=600
        )
        
        # Beautify the chart (remove the coordinate axes, only look at the distribution)
        fig.update_layout(
            xaxis=dict(showgrid=False, showticklabels=False, title=""),
            yaxis=dict(showgrid=False, showticklabels=False, title=""),
            legend=dict(title=dict(text="Segment Group"), orientation="h", y=1.05, x=0),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            font=dict(family="Oswald, sans-serif")
        )
        
        st.plotly_chart(fig, use_container_width=True)
        
        # Business Insight
        st.markdown("---")
        col_txt1, col_txt2 = st.columns(2, gap="large")
        
        # Define a simple black frame style
        box_style = """
        border: 2px solid black; 
        padding: 20px; 
        background-color: white; 
        height: 100%;
        """
        
        with col_txt1:
            st.markdown("#### :material/search: HOW TO READ THIS?")
            # Changed to a more beautiful icon :)
            st.markdown(f"""
            <div style="{box_style}">
                <ul style="margin: 0; padding-left: 20px;">
                    <li><b>The Position :</b> Determined purely by <b>Hardware Specs</b>. Similar cards are close together.The size of the bubble represents power.</li>
                    <li style="margin-top: 10px;"><b>The Color:</b> Determined by your selection (<b>AI Grouping</b> or <b>Price Tag</b>).</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
            
        with col_txt2:
            st.markdown("#### :material/lightbulb: THE STRATEGY REVEALED")
            # Changed to another nice icon again
            st.markdown(f"""
            <div style="{box_style}">
                <p><b>Try switching the view!</b> If the <b>Price Colors</b> align perfectly with the <b>Hardware Clusters</b>, it proves <b>Pricing Efficiency</b>.</p>
                <hr style="border-top: 1px dashed black; margin: 10px 0;">
                <p><b>Mismatch?</b> If you see a <span style="color:#de5253; font-weight:bold;">Red Dot</span> (Expensive) inside a <span style="color:#a1c9f4; font-weight:bold;">Blue Island</span> (Low Spec), that is an <b>overpriced product</b>!</p>
            </div>
            """, unsafe_allow_html=True)
            
    else:
        st.error("Insufficient data with Price information for clustering.")