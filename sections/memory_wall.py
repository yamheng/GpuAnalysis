...
def show(df):
    ...
    # PART 1: Interactive Sankey Diagram
    ...
    # 3. Traffic Calculation & Node Mapping
    flow = d.groupby(['Brand', 'Mem']).size().reset_index(name='Count')
    nodes = list(flow['Brand'].unique()) + list(flow['Mem'].unique())
    node_map = {n: i for i, n in enumerate(nodes)}

    # 4. Large manufacturers use more prominent color schemes
    brand_colors = {'NVIDIA': '#76B900', 'AMD': '#ED1C24', 'Intel': '#0071C5'}
    
    # Node colors: Use specific colors for brands, and black/dark gray for video memory.
    node_c = [brand_colors.get(n, 'black' if is_focus else '#333') for n in nodes]
    
    # Connection line color: brand color with transparency (focus mode) or uniform gray (chaos mode)
    if is_focus:
        ...
        # An additional library is required for hex->rgba conversion here.
